...
export async function PATCH(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const body = "dispatch" === "receive" ? await request.json().catch(() => ({})) : undefined;
    const transfer = await transitionInventoryTransfer({ request, session: guard.session, id, action: "dispatch" as never, body });
    return ok({ transfer, transferAction: "dispatch" });
  } catch (error) {
