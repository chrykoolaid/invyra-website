...
export async function PATCH(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const body = "cancel" === "receive" ? await request.json().catch(() => ({})) : undefined;
    const transfer = await transitionInventoryTransfer({ request, session: guard.session, id, action: "cancel" as never, body });
    return ok({ transfer, transferAction: "cancel" });
  } catch (error) {
